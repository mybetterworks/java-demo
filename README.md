...
当前已完成 `v0.5 Gateway JWT`。

| 项目 | 内容 |
|---|---|
| 核心能力 | 注册、登录、JWT 签发、网关 JWT 校验、获取当前登录用户、用户管理 CRUD、React 管理端、Vue 管理端、IndexedDB 登录态恢复 |
...
当前集成测试覆盖注册、重复注册、登录、JWT 查询当前用户、无 token 拦截、错误密码拦截、用户分页、详情、创建、更新、逻辑删除、修改密码和 OpenAPI JSON 生成；网关测试覆盖公开路径放行、无 token 拦截、有效 token 放行和无效 token 拦截。
...
```powershell
D:\software\jdk-17.0.19\bin\java.exe -jar backend\app\target\java-demo-app-0.5.0-SNAPSHOT.jar
```
...
```powershell
D:\software\jdk-17.0.19\bin\java.exe -jar backend\gateway\target\java-demo-gateway-0.5.0-SNAPSHOT.jar
```
...
Gateway 当前使用静态后端地址 `http://localhost:8091`，可通过环境变量 `JAVA_DEMO_BACKEND_URI` 覆盖。后续接入 Nacos 后，该地址会演进为服务发现路由。
...
Swagger UI 中可以直接查看并调试当前 v0.5 的所有后端接口。访问 `/api/users/me`、`/api/users` 这类需要登录的接口时，先调用登录接口拿到 `accessToken`，再点击页面右上角 `Authorize`，在 `bearerAuth` 中填入登录返回的 token。
...
| `JAVA_DEMO_BACKEND_URI` | `http://localhost:8091` | Gateway 转发到后端的静态地址，后续接入 Nacos 后会演进为服务发现路由 |
...
下一个 milestone 是 `v0.6 Nacos`，目标是引入服务注册与配置中心，把当前 Gateway 到后端的静态地址逐步演进为服务发现路由。
