 ...
 * Redis 基础限流服务。
 *
 * <p>本项目 v0.7 只做固定窗口限流：同一 bucket + identity 在一个短窗口内超过阈值就返回 429。
 * Redis 可用时使用 Redis INCR + EXPIRE，保证后续多实例部署时计数共享；Redis 不可用时降级到本机内存，
 * 并在日志中明确标记 fallback，避免悄悄退化。</p>
 */
@Service
public class RateLimitService {
     ...
     * 判断一次请求是否允许通过。
     *
     * @param bucket 业务桶，例如 login 或 user-query
     * @param identity 限流主体，通常是用户 ID 或客户端 IP
     * @param limit 窗口内允许请求数
     * @return true 表示放行，false 表示限流
     */
    ...
    private String redisKey(String bucket, String identity) {
        String prefix = environment.getProperty("java-demo.redis.key-prefix", "java-demo:v0_9");
