...
7. `v0.5.5` 在进入 Nacos 前继续增强登录验证码安全性，把学习型滑块升级为固定背景图随机拼图验证码。
8. `v0.6` 接入 Nacos 后，再逐步把服务发现和配置中心纳入主线。
9. `v1.5` 前后基于已有用户、任务、通知服务验证 Seata 和链路追踪，不再为了事务验证临时设计新业务。
10. 不为了“看起来像微服务”提前制造分布式复杂度。
11. 每次服务拆分后都必须保留一个可运行版本。
...
6. `v0.5.5` 起验证码本身需要升级为固定背景图随机拼图滑块：后端随机生成缺口位置并保存真实答案，前端不得获得可直接推导答案的 `targetX`。
7. 登录失败计数、验证码 challenge、拼图真实坐标、验证码图片全文和验证码 token 不得在日志中泄露真实答案、完整 token 或敏感凭证。
8. React 和 Vue 登录页必须同步支持登录风险验证和拼图滑块交互，避免只改后端导致用户无法完成登录。
9. `v0.7 Redis` 前可以使用单机 MVP 状态存储，但必须记录多实例不一致风险；`v0.7` 后优先迁移到 Redis TTL。
10. HTTPS 在 Nginx milestone 中引入，早期本地 HTTP 可接受。
