...
端口说明：本机 `7991-8090`、`8146-8245` 已被占用；当前 Windows 还保留了 `5112-5311` 段，旧 React 开发端口 `5173` 会因系统保留报 `EACCES`。React 开发端口已调整为 `5320`，Gateway 端口 `8092`、后端调试端口 `8091`、任务服务端口 `8093` 和通知服务端口 `8094` 均避开了这些范围。v0.5.1 开始，React 端的外部 API 请求统一先进入 Gateway，再由 Gateway 按路径转发到用户、任务或通知服务。
